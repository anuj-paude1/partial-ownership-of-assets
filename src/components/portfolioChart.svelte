<script>
  ...
  import { user } from '../store';
    ...
    const getChartValue = async () => {
        let res = await fetch('http://localhost:3000/investmentPart/'+ $user?.username);
        let data = await res.json();
        let total = data.realState + data.gold + data.silver
        realState = (data.realState / total) * 100
        gold = (data.gold / total) * 100
        silver = (data.silver  / total) * 100
        isLoaded = true 
    }
    getChartValue()
    $: options = {
      series: [realState, gold, silver],
      colors: ['#1C64F2', '#E6B60B', '#D1C6E2'],
      chart: {
        height: 420,
        width: '100%',
        type: 'pie'
    ...
    <div class="flex justify-between items-start w-full">
      <div class="flex-col items-center">
        <div class="flex items-center mb-1">
          <h5 class="text-xl font-bold leading-none text-gray-900 dark:text-white me-1">Portfolio Chart</h5>
